...
class TablaGuias{
    ...
    $valor_busqueda=$_POST['valor_busqueda'];
  		 ...
  		 $guias = ControladorDirProd::ctrMostrarListProductos($tipo_busqueda,$valor_busqueda);	
   ...
   $arreglo_retorno = array();
		  for($i = 0; $i < count($guias); $i++){
  		  ...
  		  $botones =  "<div class='btn-group'><button class='btn btn-warning btnEditarProducto' onclick='traspaso_variables_producto(". $concat.$guias[$i]["PRO_CODI"].$concat ."," . $concat .  str_replace('"', " ",$guias[$i]["PRO_NOMB"]) . $concat . ")'><i class='fa fa-hand-o-up'></i></button></div>"; 
