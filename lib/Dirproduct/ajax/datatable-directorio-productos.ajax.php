   ...
   $arreglo_retorno = array();
		  for($i = 0; $i < count($guias); $i++){
  		  ...
  		  $botones =  "<div class='btn-group'>
                       <button class='btn btn-primary btnEditarProducto' onclick='consultaIndividualProducto(". $concat.$guias[$i]["COD_DPRO"].$concat .")' data-toggle='modal' data-target='#modalConsultaIndividualDirPro'> <i class='fa fa-search' aria-hidden='true'></i></button>
                       <button class='btn btn-warning btnEditarProducto'  onclick='consultaIndividualProductoEdit(". $concat.$guias[$i]["COD_DPRO"].$concat .")'  data-toggle='modal' data-target='#modalConsulIndDirProEdt'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>
                       <button class='btn btn-danger btnEditarProducto'  onclick='eliminar_registro_producto(". $concat.$guias[$i]["COD_DPRO"].$concat .")'  ><i class='fa fa-window-close' aria-hidden='true'></i></button>
                               
                        </div>"; 
  		  
                  
                  $imagen =  '<img src="../'.$guias[$i]["URL_DIMA"].'?nocache='. time().'" id="'.$guias[$i]["COD_DPRO"].'" class="card-img-top-reg-'.$guias[$i]["COD_DPRO"].'"  width="100px">';
