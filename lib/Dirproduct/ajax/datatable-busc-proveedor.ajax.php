...
class TablaGuias{
    ...
    $valor_busqueda=$_POST['valor_busqueda'];
  		 ...
  		 $guias = ControladorDirProd::ctrMostrarListProveedores($tipo_busqueda,$valor_busqueda);	
   ...
   $arreglo_retorno = array();
		  for($i = 0; $i < count($guias); $i++){
  		  ...
  		  $botones =  "<div class='btn-group'><button class='btn btn-warning btnEditarProducto' onclick='traspaso_variables_proveedor(". $concat.$guias[$i]["PVD_CODA"].$concat ."," . $concat .  str_replace('"', " ",$guias[$i]["PVR_NOCO"]) . $concat . ")'><i class='fa fa-hand-o-up'></i></button></div>"; 
