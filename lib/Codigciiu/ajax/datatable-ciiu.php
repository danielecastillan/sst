...
class TablaGuias{
 ...
 /*
         ...
         $fecha=$_POST['fecha'];       
         $cont_horas_margen=$_POST['cont_horas_margen'];       
                
  		//$guias = ControladorProd::ctrMostrarProducVari($numero_producto, $nombre_producto, $punto_venta, $contador);	
  		$guias = ControladorAgent::ctrMostrarConsultasAgentTiempo($fecha);	
...
$detallesr= ''  ;       
   ...
   '.$guias[$i]["HORAS_DIA"].' 
</div>'/*  .' <input type="hidden" value="" id="" class="'.$guias[$i]["USUARIO"].'">'*/;
     $boton_verde='<div class="alert alert-success" role="alert" align="center">
   '.$guias[$i]["HORAS_DIA"].' 
</div>';
                $entrada="";
                if($guias[$i]["HORAS_DIA"]>=$cont_horas_margen){
                    
                     $entrada=$boton_verde;
                     
                     
                }
                else {  $entrada=$boton_rojo;
                
               
               $guiass = ControladorAgent::ctrRegistroErroresAgenda($guias[$i]["USUARIO"],1, $guias[$i]["USU_NOMB"] );	
              ...
              /*
                 ...
                 $arreglo_interior = array(  $guias[$i]["USUARIO"] ,
                    $guias[$i]["USU_NOMB"]    , 
                   $entrada 
