...
class TablaGuias{
	...
	public function mostrarTablaGuias(){
      //  $PVD_CODA = $_POST["PVD_CODA"];
    	...
    	$valor = null;
    	$orden = "id";
    $cod_cont_transport=$_POST['cod_cont_transport'];
 
     
  		 $guias = ControladorGuias::ctrMostrarTransportadoraGuias($cod_cont_transport);	
   ...
   $arreglo_retorno = array();
		  for($i = 0; $i < count($guias); $i++){
  		  ...
  		  $botones =  "<div class='btn-group'><button class='btn btn-warning btnEditarProducto' onclick='traspaso_variables_transportadora(". $concat.$guias[$i]["PVD_CODI"].$concat ."," . $concat . $guias[$i]["DEP_CODD"] . $concat . "," . $concat . $guias[$i]["NIT"] . $concat . "," . $concat . $guias[$i]["NOMBRE_TRANSPORTADORA"] . $concat . "," . $concat . $guias[$i]["NOMBRE_DETALLE"] . $concat . ")'><i class='fa fa-hand-o-up'></i></button></div>"; 
  		...
  		//  $botones =  "<button ></button>"; 
                //                                                                                                                                //PO_PVDOR.PVD_CODI,PO_DEPRO.DEP_CODD
                                                                                                                                                 /*
                                                                                                                                                                                                  *     $guias[$i]["NIT"],
                                                                                                                                                                                                    $guias[$i]["NOMBRE_TRANSPORTADORA"],
                                                                                                                                                                                                    $guias[$i]["DETALLE"],
                                                                                                                                                                                                    $guias[$i]["NOMBRE_DETALLE"]  
                          * 
                          * 
